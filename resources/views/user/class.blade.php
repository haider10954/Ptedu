            ...
            <div class="classroom_box mb-4 mt-2">
                <div class="left-content">
                    ...
                    <div class="content lecture_video_section">
                        @if (!empty($lecture->lecture_video))
                            <video id="player" style="height: 100%;width: 100%;" playsinline controls
                                data-poster="{{ asset('storage/course/thumbnail/' . $lecture->getSection->getCourse->course_thumbnail) }}">
                                <!-- Video files -->
                                <source src="{{ asset('storage/lectures/' . $lecture->lecture_video) }}" type="video/mp4"
                                    size="576" />
                                <source src="{{ asset('storage/lectures/' . $lecture->lecture_video) }}" type="video/mp4"
                                    size="720" />
                                <source src="{{ asset('storage/lectures/' . $lecture->lecture_video) }}" type="video/mp4"
                        ...
                        @else
                            ...
                            @endphp
                            <div id="player" data-plyr-provider="{{ $provider }}"
                ...
                <div class="right-content">
                    <div class="content-header">
                        <h6 class="mb-0 text-dark">Course Content</h6>
                    </div>
                    <div class="content">
                            ...
                            @foreach ($lectures as $record)
                                ...
                                @if ($record->slug == $lecture->slug)
                                    <li id="playingVideoLink">
                                        <a title="{{ $record->lecture_title }}" href="javascript:void(0)"
                                            class="d-flex align-items-center justify-content-between"><span>{{ $loop->index + 1 }}강.{{ \Str::limit($record->lecture_title, 25) }}</span>
                                ...
                                @else
                                    <li>
                                        <a title="{{ $record->lecture_title }}"
                                            href="{{ route('class', [$course->id, $record->slug]) }}">{{ $loop->index + 1 }}강.
