...
@section('title' , __('translation.Student offline course price control'))
...
@section('content')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0  Card_title">{{ __('translation.Student offline course price control') }}
                            ({{ $records->count()  }})</h4>
                        <div class="d-flex align-items-center gap-2">
                            <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#addRecord"><i class="bi bi-plus"></i> Add
                            </button>
                            <a class="btn btn-dark btn-sm" href="{{ route('offline-enrollments-admin',$course->id) }}">등록</a>
                        ...
                        @if($records->count() > 0)
                            @foreach($records as $item)
                                <tr>
                                    ...
                                    <td><span
                                            class="course_name">{{ (!empty($item->discounted_price)) ? $item->discounted_price : 'N/A' }}</span>
                                    </td>
                                    <td>
                                        @if($item->is_free == 1)
                ...
                <div class="col-lg-12">
                    {{ $records->links('vendor.pagination.custom-pagination-admin') }}
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5"
                            id="staticBackdropLabel">{{ __('translation.Edit Discount Entry') }}</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form method="post" action="{{ route('edit_student_offline_course_price_discount_entry') }}">
                        @csrf
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="number" class="form-control" id="edit_original_price" name="original_price"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="number" name="discounted_price" class="form-control"
                                       ...
                                       placeholder="{{ __('translation.Add discounted price') }}">
    ...
    <!-- Del Record Modal -->
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal fade" id="delRecord" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
             aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form method="post" action="{{ route('del_student_offline_course_price_discount_entry') }}">
        ...
        <div class="modal fade" id="addRecord" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5"
                            id="staticBackdropLabel">{{ __('translation.Add Discount Entry') }}</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form method="post" action="{{ route('add_student_offline_course_price_discount_entry') }}">
                        @csrf
                        <input type="hidden" name="course_id" value="{{ request()->segment(3) }}" required>
                        <div class="modal-body">
                            <div class="form-group mb-3">
